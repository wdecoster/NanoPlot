...
def get_args():
    ...
    general.add_argument(
        "--raw", help="Store the extracted data in tab separated file.", action="store_true"
    )
    general.add_argument("--huge", help="Input data is one very large file.", action="store_true")
    general.add_argument(
        "-o", "--outdir", help="Specify directory in which output has to be created.", default="."
    )
    general.add_argument("--no_static", help="Do not make static (png) plots.", action="store_true")
    general.add_argument(
        "-p",
        "--prefix",
    ...
    filtering.add_argument(
        "--maxlength", help="Hide reads longer than length specified.", type=int, metavar="N"
    )
    filtering.add_argument(
    ...
    filtering.add_argument(
        "--downsample", help="Reduce dataset to N reads by random sampling.", type=int, metavar="N"
    )
    filtering.add_argument(
        "--loglength",
    ...
    filtering.add_argument(
        "--runtime_until", help="Only take the N first hours of a run", type=int, metavar="N"
    )
    filtering.add_argument(
        "--readtype",
    ...
    visual.add_argument(
        "-c", "--color", help="Specify a valid matplotlib color for the plots", default="#4CB391"
    )
    visual.add_argument(
        "-cm",
        "--colormap",
    ...
    visual.add_argument(
        "--legacy",
        help="Specify which bivariate plots have to be made (legacy mode).",
        type=str,
        nargs="*",
        choices=["kde", "dot", "hex"],
    )
    ...
    visual.add_argument(
        "--no-N50", help="Hide the N50 mark in the read length histogram", action="store_true"
    )
    visual.add_argument(
        "--N50",
    ...
    mtarget.add_argument(
        "--fastq", help="Data is in one or more default fastq file(s).", nargs="+", metavar="file"
    )
    mtarget.add_argument(
        "--fasta", help="Data is in one or more fasta file(s).", nargs="+", metavar="file"
    )
    mtarget.add_argument(
        "--fastq_rich",
    ...
    mtarget.add_argument(
        "--bam", help="Data is in one or more sorted bam file(s).", nargs="+", metavar="file"
    )
    mtarget.add_argument(
        "--ubam", help="Data is in one or more unmapped bam file(s).", nargs="+", metavar="file"
    )
    mtarget.add_argument(
        "--cram", help="Data is in one or more sorted cram file(s).", nargs="+", metavar="file"
    )
